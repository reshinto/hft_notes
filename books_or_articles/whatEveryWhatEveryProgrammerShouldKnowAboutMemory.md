...
- **Throughput vs latency:** The bus can burst at high rates, but **gaps from tRCD/tRP/tRAS** slash sustained bandwidth unless accesses reuse open rows and burst multiple words. *(§2.2.1–§2.2.2)*
- **Refresh matters:** DRAM must refresh about **every 64 ms**; with **8192 rows** that’s ~**7.8125 µs** between refreshes on average, blocking access to the refreshed row. *(§2.2.3)*
- **Generations:** **SDR → DDR1 (double data rate) → DDR2 (faster bus + 4-bit I/O buffer) → DDR3 (lower V, 8-bit I/O buffer)**. Names (PC/DDR-xxxx) reflect **effective** rates. *(§2.2.4, Tables 2.1–2.2)*
- **FB-DIMM (FB-DRAM):** Serial, full-duplex channel cuts pins (≈**69 vs 240**), allows **more channels & DIMMs**, and higher aggregate bandwidth; trade-offs: extra latency per hop and higher module power. *(§2.2.4, FB-DRAM table)*
- **Other memory users:** **DMA** devices contend for memory/FSB bandwidth; NUMA layouts can isolate DMA vs compute memory. Integrated-graphics sharing system RAM hurts latency. *(§2.3)*
...
- **Key insight:** Row reuse (keeping it “open”) minimizes RAS work; random row switches amplify latency. *(§2.2.1)*

#### Timing Symbols (appear in specs like **w-x-y-z-T**, e.g., **2-3-2-8-T1**) *(§2.2.2)*
...
- **Illustrative occupancy:** Example shows **2 cycles of data** out of **7 total cycles** → the 800 MHz-effective × 8 B burst **6.4 GB/s** shrinks to ~**1.8 GB/s** (6.4 × 2/7). *(§2.2.2)*  
- **Takeaway:** Align access patterns to reduce precharge/activate churn (reuse rows, longer bursts). *(§2.2.1–2.2.2)*

#### 3) Refresh (Recharging) *(§2.2.3)*
...
- **Burst bandwidth formula:** `BW_theoretical = (bus_width_bytes) × (effective_bus_freq)`  
  - Example in text: **8 B × 800 MHz = 6.4 GB/s** (quad-pumped 200 MHz). *(§2.2)*
- **Sustained bandwidth** depends on **occupancy** (fraction of time the bus actually transfers data).  
  - Example: **2/7 occupancy ⇒ 6.4 × 2/7 ≈ 1.8 GB/s**. *(§2.2.2)*

#### 5) SDR → DDR1/DDR2/DDR3 (Figs. 2.10–2.13; Tables 2.1–2.2)
**Core ideas:**
...
- **DDR2:** Bus at **2f**, 4-bit I/O buffer; array still at **f**. Names reflect 4× factor (quad-pumped). *(Fig. 2.12, Table 2.1)*
- **DDR3:** Lower voltage (**~1.5 V vs 1.8 V** → ~30% power reduction by V²), bus faster, **8-bit I/O buffer**; initially higher CL but enables higher bandwidth; naming per Table 2.2. *(Fig. 2.13, §2.2.4)*

**Naming examples from tables:**
- **DDR2**: e.g., **PC2-6400** (6,400 MB/s) a.k.a. **DDR2-800** (bus 400 MHz, effective 800 MHz). *(Table 2.1)*
- **DDR3**: e.g., **PC3-12800** a.k.a. **DDR3-1600** (bus 800 MHz, effective 1600 MHz). *(Table 2.2)*

**Practical limits (from text):**
- Many pins: ~**240 pins**/DIMM and signal-integrity constraints → typically **≤2 DIMMs/channel (DDR2)** and **often 1 DIMM/channel at high DDR3 rates**. This caps capacity per controller unless more channels/controllers/IMCs are used. *(§2.2.4)*

#### 6) FB-DRAM (Fully Buffered DIMM) Highlights *(§2.2.4 + table)*
...
#### 7) Other Main-Memory Users (DMA) *(§2.3)*
...
- **Q:** What do the five timing numbers `w-x-y-z-T` mean?  
  **A:** `CL, tRCD, tRP, tRAS, CommandRate`. *(§2.2.2)*
- **Q:** Why does DDR increase bandwidth but not intrinsic latency?  
  **A:** It transfers **more per cycle** (edges / faster bus / wider I/O buffer), but **RC physics and RAS/CAS** still set latency. *(§2.2.1, §2.2.4)*
- **Q:** Why can “random” access crush bandwidth?  
  **A:** Each new row needs **precharge (tRP)** + **activate (tRCD)** + **CL**, leaving the bus idle between bursts. *(§2.2.2)*
- **Q:** What’s the refresh interval per row for 8192 rows?  
  **A:** **64 ms / 8192 ≈ 7.8125 µs**. *(§2.2.3)*

#### Quick Math
- **Theoretical burst:** `8 B × 800 MHz = 6.4 GB/s` (quad-pumped 200 MHz bus). *(§2.2)*  
- **Observed in example:** `6.4 × (2/7) ≈ 1.8 GB/s` when timing gaps dominate. *(§2.2.2)*  
- **DDR3 power note:** `P ∝ V²` → from **1.8 V → 1.5 V** ≈ **(1.5² / 1.8²) ≈ 0.69** → ~**30% reduction**. *(§2.2.4)*
...
- **Prefetch:** Use hardware/soft prefetch to overlap timing gaps and shift contention. *(§2.2.5)*  
- **NUMA-aware placement:** Keep threads near their data; separate DMA-heavy devices from compute memory when possible. *(§2.3)*  
